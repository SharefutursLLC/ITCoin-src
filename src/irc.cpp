...
// Copyright (c) 2011-2012 ITCoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ITCoinTEST0\r");
            Send(hSocket, "WHO #ITCoinTEST0\r");
        } else {
            // randomly join #ITCoin00-#ITCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #ITCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #ITCoin%02d\r", channel_number).c_str());
        }
