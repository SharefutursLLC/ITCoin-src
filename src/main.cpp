...
uint256 hashGenesisBlock("0xd767a3ae4cb1e97be9a5acedec0eb956a1e41c4149dd8541f576b27ca86ef1d1");
...
const string strMessageMagic = "ITCoin Signed Message:\n";
...
{
    // ITCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // ITCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "it genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ITCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ITCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ITCoinMiner(CWallet *pwallet)
{
    printf("ITCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ITCoin-miner");
    ...
    try { loop {
        ...
        printf("Running ITCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ITCoinMiner, pwallet));
}
